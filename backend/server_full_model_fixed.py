...
def optimize():
    ...
    prod_rate = {(p['name'], b['name']): float(p['productionRates'].get(b['name'], 0))
                 for p in products for b in brigades}
    qualification = {(p['name'], b['name']): int(p['qualifications'].get(b['name'], 0))
                     for p in products for b in brigades}
    ...
    def plan_rule(m, p):
        qty = sum((m.xs[p, b] + m.xns[p, b]) * prod_rate[p, b]
                  for b in m.B if (p, b) in prod_rate)
    ...
    def sl_required(m, p):
        total_output = sum((m.xs[p, b] + m.xns[p, b]) * prod_rate[p, b]
                           for b in m.B if (p, b) in prod_rate)
    ...
    solver.solve(model)
    по_бригадам, загрузка = {}, {b: {'часы': 0, 'лимит': load[b]} for b in brigade_names}
    производство, task_hours_by_brigade = {}, {b: [] for b in brigade_names}

    for p in product_names:
        производство[p] = {'план': serial_plan[p] + nonserial_plan[p], 'факт': 0}
        for b in brigade_names:
            val_s = model.xs[p, b].value or 0
            val_ns = model.xns[p, b].value or 0
            qty = (val_s + val_ns) * prod_rate.get((p, b), 0)
            часы = val_s + val_ns
            if часы > 0:
    ...
    for b in загрузка:
        часы, лимит = загрузка[b]['часы'], загрузка[b]['лимит']
        загрузка[b]['загрузка_%'] = round(100 * часы / лимит, 1) if лимит > 0 else 0
    for p in производство:
        производство[p]['факт'] = round(п := производство[p]['факт'], 2)
        производство[p]['отклонение'] = round(план := производство[p]['план'] - п, 2)

    # 📆 График на 90 дней по 2/2
    start_date = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)
    days = [start_date + timedelta(days=i) for i in range(90)]
    schedule_matrix = {b: [] for b in brigade_names}
    for i, b in enumerate(brigade_names):
    ...
    for b in brigade_names:
        ...
        for date in schedule_matrix[b]:
            remain = max_hours_per_day
            i = 0
            while i < len(remaining_tasks) and remain > 0:
                task = remaining_tasks[i]
                use = min(task['hours'], remain)
                schedule_output[b].append({'date': date, 'product': task['product'], 'hours': use})
                task['hours'] -= use
                remain -= use
                if task['hours'] <= 0:
